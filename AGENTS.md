...
# TanStack Agentic Docs Flow (MCP replacement)

NOTE: `tanstack mcp` was REMOVED from the TanStack CLI and will not be restored.
Do NOT configure an MCP server pointing to `@tanstack/cli mcp` — it does not exist.
Use the TanStack CLI directly for docs/introspection (all commands support `--json`):

```bash
bunx @tanstack/cli@latest libraries --json                                  # list TanStack libraries
bunx @tanstack/cli@latest search-docs "<query>" --library start --framework react --json   # search docs
bunx @tanstack/cli@latest doc start framework/react/<path> --json           # fetch a docs page
bunx @tanstack/cli@latest ecosystem --json                                  # ecosystem partners
bunx @tanstack/cli@latest create --list-add-ons --framework React --json    # list add-ons
```

Combine with the Intent skill mappings above: run the matching
`bunx @tanstack/intent@latest load <skill-id>` command before editing files.

<!-- vercel-react-best-practices:start -->
# Vercel React Best Practices (project skill)

This project includes the `vercel-react-best-practices` skill (Vercel Engineering,
v1.0.0, MIT) at `.agents/skills/vercel-react-best-practices/`.

Apply it when writing, reviewing, or refactoring React/Next.js code: components,
pages, data fetching, bundle optimization, or performance work.

- Start with `.agents/skills/vercel-react-best-practices/SKILL.md` (rule index,
  70 rules across 8 categories, prioritized by impact).
- Read individual rules from `.agents/skills/vercel-react-best-practices/rules/<rule-id>.md`
  (each has incorrect/correct examples).
- Full compiled guide: `.agents/skills/vercel-react-best-practices/AGENTS.md`.

Category priority: 1. Eliminating Waterfalls (`async-`, CRITICAL) >
2. Bundle Size (`bundle-`, CRITICAL) > 3. Server-Side (`server-`, HIGH) >
4. Client Data Fetching (`client-`, MEDIUM-HIGH) > 5. Re-render (`rerender-`, MEDIUM) >
